...
public class DMDatabase extends AbstractJdbcDatabase {
        ...
        if (!(conn instanceof OfflineConnection)) {
            ...
            if (sqlConn != null) {
                ...
                try {
                    ...
                    //noinspection HardCodedStringLiteral
                    Scope.getCurrentScope().getLog(getClass()).info("Could get sql keywords on DMDatabase: " + e.getMessage());
                ...
                try {
                    ...
                    //noinspection HardCodedStringLiteral
                    Scope.getCurrentScope().getLog(getClass()).info("Could not set remarks reporting on DMDatabase: " + e.getMessage());

                    //cannot set it. That is OK
                }

                //TODO:暂时不知道如何获取达梦数据库版本，及设置ddl锁定超时时间
                ...
                //
                //     //noinspection HardCodedStringLiteral
                //     Scope.getCurrentScope().getLog(getClass()).info("Could not set check compatibility mode on DMDatabase, assuming not running in any sort of compatibility mode: " + message);
                // } finally {
    ...
    public boolean isSystemObject(DatabaseObject example) {
        ...
        if (example instanceof Schema) {
            //noinspection HardCodedStringLiteral,HardCodedStringLiteral,HardCodedStringLiteral,HardCodedStringLiteral
            if ("CTISYS".equals(example.getName()) || "SYS".equals(example.getName()) || "SYSAUDITOR".equals(example.getName()) || "SYSDBA".equals(example.getName()) || "SYSSSO".equals(example.getName())) {
                return true;
            }
            //noinspection HardCodedStringLiteral,HardCodedStringLiteral,HardCodedStringLiteral,HardCodedStringLiteral
            if ("CTISYS".equals(example.getSchema().getCatalogName()) || "SYS".equals(example.getSchema().getCatalogName()) || "SYSAUDITOR".equals(example.getSchema().getCatalogName()) || "SYSDBA".equals(example.getSchema().getCatalogName()) || "SYSSSO".equals(example.getSchema().getCatalogName())) {
                return true;
            }
        ...
        if (example instanceof Catalog) {
            ...
            if (example.getName().equals("SYSCONTEXTINDEXES")) { //CTISYS 系统表
                return true;
            }
            else if (example.getName().startsWith("AQ$")) { //SYS AQ tables
                return true;
            }
            else if (example.getName().startsWith("##TMP_TBL_FOR_DBMS_LOB_")) { //SYS TMP_TBL_FOR_DBMS_LOB_BLOB/TMP_TBL_FOR_DBMS_LOB_CLOB tables
                return true;
            }
            else if (example.getName().equals("DBMS_ALERT_INFO") || example.getName().equals("DBMS_LOCK_ALLOCATED")) { //SYS DBMS_ALERT_INFO/DBMS_LOCK_ALLOCATED tables
                return true;
            }
            else if (example.getName().equals("POLICIES") || example.getName().equals("POLICY_COLS") || example.getName().equals("POLICY_CONTEXTS") || example.getName().equals("POLICY_GROUPS")) { //SYS POLICIES/POLICY_COLS/POLICY_CONTEXTS/POLICY_GROUPS tables
                return true;
            }
            else if (example.getName().startsWith("REG$")) { //SYS REG$ tables
                return true;
            }
            else if (example.getName().startsWith("V$")) { //SYSAUDITOR V$AUDITRECORDS/V$AUDIT_SPACE tables
                return true;
            }
            else if (example.getName().equals("##HISTOGRAMS_TABLE") || example.getName().equals("##PLAN_TABLE")) { //SYSDBA ##HISTOGRAMS_TABLE/##PLAN_TABLE tables
                return true;
            }
        }
